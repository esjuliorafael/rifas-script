...
<script>
    // Al cargar la página, restauramos la pestaña activa
    document.addEventListener('DOMContentLoaded', () => {
        const lastTab = localStorage.getItem('last_config_tab');
        if (lastTab) {
            // Buscamos el botón correspondiente
            const btn = document.getElementById('tab-btn-' + lastTab);
            if (btn) {
                switchTab(lastTab, btn);
            }
        }
    });

    function switchTab(panelId, btnElement) {
        // 1. Guardar estado en memoria del navegador
        localStorage.setItem('last_config_tab', panelId);

        // 2. Lógica visual (Ocultar/Mostrar)
